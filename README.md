...
- 指令可转换为结构化操作，并真实写入项目、产品、项目策略、项目状态、产品事实、项目提醒、渠道表现、内容日历、素材包、审核任务、首月计划或进入待确认状态；当前支持新项目启动、项目切换、项目简报、当前项目新增并关联产品、市场、渠道、客群、内容方向、素材包频率、项目状态、策略确认、产品事实新增/提取/修改/确认、提醒创建/改期/完成/忽略、指标单条录入/批量导入、风险提醒生成、计划项新增/改期/改状态/完成、指定周期素材包创建/归档/恢复草稿、素材包文件推进、模板海报关联、素材包提交审核、审核任务处理/取消/补齐和首月计划生成。
...
- `/b-agent` 左侧中文指令可粘贴 CSV 或表格文本批量导入渠道表现，指标会写入当前项目的数据复盘和变更日志。
