...
- 素材库支持上传真实产品图、官方 Logo、资料和参考图，并保存审核状态、校验和、本地存储路径。
- 图片生成供应商配置和生成任务已建立数据库底座，当前只保存候选配置和任务记录，不调用真实模型。
- 内容日历、素材包中心、审核中心、提醒中心已读取数据库，不再只是占位。
- 素材包中心支持本地 ZIP 清单导出，包含说明、排期 CSV、文案草稿、Hashtags、脚本、设计 Brief、合规检查和 manifest。
- 今日工作台展示项目、产品、计划、素材包、提醒和最近变更。
- 集成设置支持保存飞书占位连接、停用旧连接和记录换绑/迁移说明，但不接真实飞书 API。
- 数据复盘基于系统内策略、计划、素材、提醒、指令和变更日志生成本地摘要。
