...
- `/b-agent` 也是多项目启动入口：已有项目时仍可用中文 Brief 创建新项目，复用项目中心的启动事务并跳转到新项目工作台。
- `/b-agent` 复用项目就绪度算法展示评分、阻塞项和下一步动作，避免用户必须跳到今日工作台或项目详情才知道项目是否可推进。
- 中文指令通过 `AgentTextProvider` 进入系统；当前启用本地规则型 `local-rule` Provider，不调用真实 GPT，已支持新项目启动、当前项目新增并关联产品、市场、渠道、客群、内容方向、素材包频率、项目状态、产品事实提取、计划、素材包、审核、复盘指标和提醒。
